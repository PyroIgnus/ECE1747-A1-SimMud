...
void WorldUpdateModule::run()
{
    ...
    Uint32 req_count, req_time, req_start;
    Uint32 upd_count, upd_time, upd_start;
    req_count = 0; req_time = 0;
    upd_count = 0; upd_time = 0;
	...
	while ( true )
	{
        ...
        while( (m = comm->receive( timeout, t_id )) != NULL )
        {
            ...
            req_time += SDL_GetTicks() -  req_start;
        }
	    ...
	    while ( ( p = bucket->next() ) != NULL )
	    {
            ...
            upd_time += SDL_GetTicks() - upd_start;
	    }
        ...
        log->write(t_id, SDL_GetTicks(), req_count, req_time, upd_count, upd_time);
	}
}
