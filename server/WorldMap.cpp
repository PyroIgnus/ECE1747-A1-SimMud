...
void WorldMap::generate()
{
	...
	safe_level = (sd->overloaded_level + sd->light_level) / 2;
	...
	tStats = new ThreadStatistics[sd->num_threads];
	
	n_regs.x = size.x / regmin.x;
	n_regs.y = size.y / regmin.y;
	int regions_per_thread = (n_regs.x * n_regs.y - 1) / sd->num_threads + 1;	// We can have multiple regions per thread
	...
	for( i = 0, pos.x = 0; i < n_regs.x; i++, pos.x += regmin.x )
	{
		...
		for( j = 0, pos.y = 0; j < n_regs.y; j++, pos.y += regmin.y ) {
			int layout = (i*n_regs.y + j) / regions_per_thread;
			initRegion( &regions[i][j], pos, regmin, layout, objs, pls);
			// Initialize thread statistics values
			tStats[layout].number_of_regions += 1;
			tStats[layout].number_of_players = 0;
			tStats[layout].players_in_most_crowded_region = 0;
		}
	}
...
Player* WorldMap::addPlayer( IPaddress a )
{
	...
	tStats[r->layout].number_of_players += 1;
...
void WorldMap::movePlayer(Player* p)
{
	...
	if( r_old->layout != r_new->layout && res )
	{
		...
		tStats[r_new->layout].number_of_players += 1;
		tStats[r_old->layout].number_of_players -= 1;
	}
}
...
/**
 * Load is defined by number of players
 * this assumes that every player has approximately
 * the same amount of activity as another
 */
...
bool WorldMap::isOverloaded( int n_pl ) {
	...
	//TODO: Determine how to interpret loading level (specifically overloaded_level)
...
/**
 * Reassigns regions from one thread to another and returns the number of regions that were shed
 */
int WorldMap::shedRegionsToThread(int new_layout, int old_layout) {
	int numRegionsShed = 0;
	// Use simplistic algorithm to scan regions in order and place them into a bin to be shed to new node
	int i, j;
	Vector2D pos;
	for( i = 0, pos.x = 0; i < n_regs.x; i++, pos.x += regmin.x ) {
		for( j = 0, pos.y = 0; j < n_regs.y; j++, pos.y += regmin.y ) {
...
{
	// Minimize difference between highest and lowest loaded servers
	// Use permutations to brute force determine best mapping of regions?
}
